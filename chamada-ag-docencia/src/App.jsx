...
function Main({ session }) {
  ...
  return (
    <div className={'wrap' + (tab === 'analise' ? ' wide' : '')}>
      ...
      <nav className="tabs">
        {[['chamada', 'Chamada'], ['missoes', 'Missões'], ['insignias', 'Insígnias'], ['avisos', 'Avisos'], ['analise', 'Análise'], ['radar', 'Radar'], ['mapa', '🗺️ Mapa'], ['conferir', 'Conferir faltantes'], ['resumo', 'Resumo / Exportar'], ['posicao', 'Minha posição'], ['turmas', 'Turmas & Fotos']].map(([k, l]) =>
    ...
    <div className="panel">
      ...
      {!sessao?.aberta && <>
        <div className="row">
          ...
          <div><label className="fld">Local da aula</label>
      ...
      {sessao?.aberta && <>
        <div className="codigo-box">
          <div className="cb-lab">Código da aula — projete esta tela{sessao.local ? ' · local: ' + sessao.local : ''}{sessao.tempo ? ' · céu: ' + sessao.tempo : ''}
...
function Resumo({ tid, turmas, showToast }) {
  ...
  function baixar(lines, prefixo) {
    ...
    const a = document.createElement('a'); a.href = url; a.download = prefixo + t.nome.replace(/[^\w\-]+/g, '_') + '.csv'
